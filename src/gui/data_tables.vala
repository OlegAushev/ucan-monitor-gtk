...
extern void motordrive_get_watch_value(string name, string value);
...
public class DataTables : Adw.Bin
{
	...
	{
		string result = "";
		motordrive_get_watch_value("UPTIME", result);
		entryUptime.entry_text = result;

		motordrive_get_watch_value("DRIVE_STATE", result);
		entryState.entry_text = result;

		motordrive_get_watch_value("FAULTS", result);
		entryErrors.entry_text = result;

		motordrive_get_watch_value("WARNINGS", result);
	...
	{
		string result = "";
		motordrive_get_watch_value("SPEED_RPM", result);
		entrySpeed.entry_text = result;

		motordrive_get_watch_value("TORQUE", result);
		entryTorque.entry_text = result;

		motordrive_get_watch_value("STATOR_CURRENT", result);
		entryCurrS.entry_text = result;

		motordrive_get_watch_value("FIELD_CURRENT", result);
		entryCurrF.entry_text = result;

		motordrive_get_watch_value("D_CURRENT", result);
		entryCurrD.entry_text = result;

		motordrive_get_watch_value("Q_CURRENT", result);
		entryCurrQ.entry_text = result;

		motordrive_get_watch_value("GAMMA_ANGLE_DEG", result);
		entryGamma.entry_text = result;

		motordrive_get_watch_value("MOTOR_S_TEMP", result);
		entryTempS.entry_text = result;

		motordrive_get_watch_value("MOTOR_FW_TEMP", result);
		entryTempFW.entry_text = result;

		motordrive_get_watch_value("MECH_POWER", result);
	...
	{
		string result = "";
		motordrive_get_watch_value("DC_VOLTAGE", result);
		entryVoltDC.entry_text = result;

		motordrive_get_watch_value("DC_CURRENT", result);
		entryCurrDC.entry_text = result;

		motordrive_get_watch_value("PHA_CURRENT", result);
		entryCurrPhA.entry_text = result;

		motordrive_get_watch_value("PHB_CURRENT", result);
		entryCurrPhB.entry_text = result;

		motordrive_get_watch_value("PHC_CURRENT", result);
		entryCurrPhC.entry_text = result;

		motordrive_get_watch_value("PHA_TEMP", result);
		entryTempPhA.entry_text = result;

		motordrive_get_watch_value("PHB_TEMP", result);
		entryTempPhB.entry_text = result;

		motordrive_get_watch_value("PHC_TEMP", result);
		entryTempPhC.entry_text = result;

		motordrive_get_watch_value("CASE_TEMP", result);
		entryTempAir.entry_text = result;

		motordrive_get_watch_value("OUT_ELEC_POWER", result);
