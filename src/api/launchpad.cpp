...
void launchpad_set_client_value(unsigned int tpdo_num, double value) {
    assert(tpdo_num <= 3);
    launchpad_server->set_client_value(static_cast<ucanopen::CobTpdo>(tpdo_num), value);
}


void launchpad_set_server_value(unsigned int rpdo_num, double value) {
    assert(rpdo_num <= 3);
    launchpad_server->set_server_value(static_cast<ucanopen::CobRpdo>(rpdo_num), value);
}
